...
import com.szo.core.util.BrowserUtils;
import com.szo.core.util.ExceptionUtil;
import com.szo.core.util.StringUtil;
import com.szo.core.util.excel.ExcelExportUtil;
import com.szo.core.util.excel.ExcelUtil;
import com.szo.core.util.excel.entity.ExcelTitle;
import com.szo.module.demo.entity.test.JpPersonEntity;
...
public class JpPersonController extends BaseController {
    ...
    private static final Logger logger = Logger
            .getLogger(JpPersonController.class);
    ...
    public ModelAndView jpPerson(HttpServletRequest request) {
        return new ModelAndView("szo/demo/test/jpPersonList");
    }
    ...
    public void exportXls(HttpServletRequest request,
        ...
        try {
            ...
            List<JpPersonEntity> jpPersons = this.jpPersonService
                    .loadAll(JpPersonEntity.class);
            workbook = ExcelExportUtil.exportExcel(new ExcelTitle(null, null,
                    "导出信息"), JpPersonEntity.class, jpPersons);
    ...
    public ModelAndView goImplXls(HttpServletRequest request) {
        return new ModelAndView("szo/demo/test/upload");
    }

    // 老的EasyUI上传方式（2013/05/28废弃）
    @RequestMapping(params = "implXls")
    @ResponseBody
    public AjaxJson implXls(HttpServletRequest request,
                            HttpServletResponse response) {
        ...
        try {
            ...
            listPersons = (List<JpPersonEntity>) ExcelUtil.importExcelByIs(
                    file.getInputStream(), JpPersonEntity.class);
            for (JpPersonEntity person : listPersons) {
                person.setId(UUID.randomUUID().toString());
                if (person.getAge() == null || person.getCreatedt() == null
    ...
    public AjaxJson del(JpPersonEntity jpPerson, HttpServletRequest request) {
        AjaxJson j = new AjaxJson();
        jpPerson = systemService.getEntity(JpPersonEntity.class,
                jpPerson.getId());
        message = "删除成功";
    ...
    public ModelAndView addorupdate(JpPersonEntity jpPerson,
                                    HttpServletRequest req) {
        if (StringUtil.isNotEmpty(jpPerson.getId())) {
            jpPerson = jpPersonService.getEntity(JpPersonEntity.class,
                    jpPerson.getId());
            req.setAttribute("jpPersonPage", jpPerson);
        }
        return new ModelAndView("szo/demo/test/jpPerson");
    }
}

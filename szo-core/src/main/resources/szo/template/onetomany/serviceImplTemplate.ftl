...
public class ${entityName}ServiceImpl extends CommonServiceImpl implements ${entityName}ServiceI {
	...
	public void addMain(${entityName}Entity ${entityName?uncap_first},
			...
			for(${sub.entityName}Entity ${sub.entityName?uncap_first}:${sub.entityName?uncap_first}List){
				...
				${sub.entityName?uncap_first}.set${key}(${entityName?uncap_first}.get${szo_table_id?cap_first}());
				<#else>
	...
	public void updateMain(${entityName}Entity ${entityName?uncap_first},
	    ...
	    List<${sub.entityName}Entity> ${sub.entityName?uncap_first}OldList = this.findHql(hql${sub_index},<#list sub.foreignKeys as key><#if key?lower_case?index_of("${szo_table_id}")!=-1>${szo_table_id}${sub_index}<#else>${key?uncap_first}${sub_index}</#if><#if key_has_next>,</#if></#list>);
